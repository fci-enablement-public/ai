...
printf "%-30s%-30s\n" "Configure LDAP client 1/6" "Install an LDAP client like Softerra LDAP Browser or ldapsearch (yum install openldap-clients)"
printf "%-30s%-30s\n" "Configure LDAP client 2/6" "Point your LDAP client here: ldaps://fcai-openldap:30636"
printf "%-30s%-30s\n" "Configure LDAP client 3/6" "In your LDAP client's /etc/hosts (or equivalent), add this entry (modify IP if needed): ${IP} fcai-openldap"
printf "%-30s%-30s\n" "Configure LDAP client 4/6" "From Kubernetes, run this command and add the resulting cert into your LDAP client: cat ${base}/ldap.crt"
printf "%-30s%-30s\n" "Configure LDAP client 5/6" "For RedHat/CentOS/Fedora clients, run this: echo \"TLS_CACERT /etc/openldap/certs/trusted.pem\" >> /etc/openldap/ldap.conf"
printf "%-30s%-30s\n" "Configure LDAP client 6/6" "For RedHat/CentOS/Fedora clients, copy and paste step 4's output into /etc/openldap/certs/trusted.pem"
